/**
 * Seed script para crear citas de prueba para HOY (20 nov) y MAÑANA (21 nov 2025)
 * 
 * Características:
...
async function deleteAllAppointments() {
  ...
  console.log(`✅ Eliminadas ${deletedCount} citas (${scanResult.Items.filter(i => i.type === 'personal').length} personales, ${scanResult.Items.filter(i => i.type !== 'personal').length} de negocio)`);
}
...
async function createAppointmentsForDate(date: string, dateLabel: string, startCustomerIndex: number) {
  ...
  for (const appointment of nonOptimalRoute) {
    ...
    const startTime = `${date}T${appointment.time}:00-05:00`;
...
async function main() {
  try {
    console.log('🚀 Iniciando seed de citas para HOY (20 nov) y MAÑANA (21 nov 2025)...\n');
    
    // 1. Eliminar citas existentes
    await deleteAllAppointments();
    
    // 2. Crear citas para HOY (20 de noviembre)
    const todayCount = await createAppointmentsForDate(TODAY, 'HOY', 0);
    
    // 3. Crear citas para MAÑANA (21 de noviembre)
